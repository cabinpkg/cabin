...
- `include-dirs`: public include directories, relative to the package root.  Absolute paths and
  `..` traversal are rejected when the manifest loads: consumers inherit these dirs, so a package
  must not aim their include search outside itself.  Consumers of this target inherit them through
  `deps`.
