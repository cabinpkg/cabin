...
This document covers the target kinds Cabin supports today, when
each one is built, and where their outputs land.
...
- no `cabin run --example`, and no single-example selector on
  `cabin build` — `example` targets only reach the build
