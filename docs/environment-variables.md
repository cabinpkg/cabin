...
| `CABIN_REGISTRY_TOKEN` | unset | Bearer token for registry access.  When set and non-empty it wins over the stored login session for the default hosted registry's origin and for a loopback origin the user chose (`--index-url` or user-level config); other registries - and a loopback origin a checked-out project's config or a `[source-replacement]` hop picked - always use the stored session.  See [`remote-registry.md`](remote-registry.md#environment-override).  The repository's registry-admin maintenance commands (`cargo registry-governor`, `cargo registry-diagnose`, the backup audit) read the same variable as their operator credential. |
...
| `CABIN_GITHUB_OAUTH_URL` | `https://github.com` | Test seam: the GitHub OAuth host `cabin login`'s [device flow](remote-registry.md#cabin-login-and-cabin-logout) contacts.  Cabin's own tests point it at a loopback mock; there is no production reason to set it. |
