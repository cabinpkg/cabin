...
| `CABIN_REGISTRY_TOKEN` | unset | Bearer token for registry access.  When set and non-empty it wins over `credentials.toml` for the default hosted registry's origin and for a loopback origin the user chose (`--index-url` or user-level config); other registries - and a loopback origin a checked-out project's config or a `[source-replacement]` hop picked - always use `credentials.toml`.  See [`remote-registry.md`](remote-registry.md#environment-override). |
