...
- `cabin new <path>` creates a new directory at `<path>` and fills it with the scaffold.  `<path>`
  must not already exist.
- `cabin init` initializes the current directory.  The directory may already contain user files; the
  scaffold never overwrites them.
...
collaborators and CI converge on the same resolution; see [`lockfile.md`](lockfile.md).

## Package name

When `--name` is omitted, the package name is derived from the final path component (`cabin new
path/to/hello-world` produces `hello-world`).  If that component is empty, contains characters
...
- a destination that already exists (for `cabin new` - the message suggests `cabin init` for that
  case);
