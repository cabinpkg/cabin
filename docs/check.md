...
is **incremental and parallel**: a second run with no source changes
does nothing, and editing a header re-checks only the translation
units that include it. It is the fast inner-loop command for catching
compile errors before paying for a full build.
...
`cabin check` accepts the same options as `cabin build` — manifest and
...
Cabin plans the build for the selected package(s), rewrites every
...
A check honors the same profile selection as `cabin build`, defaulting
to `dev`. The profile matters for a syntax check: its flags (for
example `-O3 -DNDEBUG` under `release`) can change which `#if`
branches compile. See [Build profiles](profiles.md).
...
`-j` / `--jobs` caps how many checks run concurrently, with the same
...
target set [`cabin build`](index.md#quickstart) builds.
...
[foundation ports](foundation-ports.md): your code never compiles a
...
Header-only targets declare no translation units, so they emit no
check of their own; their headers are validated through the targets
that include them.
...
- **`cabin build`** compiles each translation unit to an object,
  archives libraries, and links executables — producing runnable
  artifacts.
- **`cabin check`** compiles each translation unit with
  `-fsyntax-only` (parse plus semantic analysis, no code generation)
  and stops there — producing only diagnostics.
...
[`cabin fmt`](fmt.md) (formatting) and [`cabin tidy`](tidy.md)
(static analysis).
