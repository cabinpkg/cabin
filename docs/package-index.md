...
Local-only projects (no versioned dependencies) require neither
flag.
...
[`registry-design.md`](registry-design.md) for the full layout
contract.
...
- platform-specific dependency data beyond the current serialized
  dependency records;
- mirror configuration;
- a cabin-specific JSON schema document; the format is documented
  here and validated by code, but no formal `$schema` URL is
  published.
...
- Absolute or scheme-relative `http://` / `https://` values are
  ...
  Cross-origin artifact URLs and URLs containing `userinfo`
  credentials are rejected before any download is attempted.
...
commands documented in [`vendoring-offline.md`](vendoring-offline.md).
