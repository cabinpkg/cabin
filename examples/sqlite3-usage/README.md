...
foundation port.  The program links against SQLite (the single-file amalgamation), opens an
in-memory database, runs a query, and prints the library version and thread-safety mode.

This is **not** itself a port and does not vendor or copy SQLite sources.  It demonstrates depending
on a curated foundation port from a normal Cabin package.  The first `cabin build` downloads the
upstream amalgamation archive (URL and SHA-256 pinned by the port recipe) from `sqlite.org`,
verifies its checksum, extracts it under Cabin's cache, and then builds normally; subsequent builds
reuse the cache.
...
The port builds threadsafe (serialized) SQLite by default.  On Unix that needs `-lpthread -ldl -lm`,
which the port declares as propagating `link-libs` so this consumer links them automatically.

To compile a single-threaded SQLite instead - dropping the threading layer via `SQLITE_THREADSAFE=0`
- enable the port's `single-threaded` feature on the dependency:
...
(`crates/cabin/tests/cabin_examples.rs::sqlite3_usage_builds_and_runs`) skips cleanly when
