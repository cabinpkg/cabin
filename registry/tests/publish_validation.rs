...
//!   `cargo gen-fixtures` (the in-tree cabin binary) through the same
...
/// `cargo gen-fixtures <dir>` first and point
/// `CABIN_REGISTRY_FIXTURES` at `<dir>`.
#[test]
#[ignore = "needs CABIN_REGISTRY_FIXTURES pointing at gen-fixtures output"]
fn generated_fixtures_pass_the_full_validation_path() {
    let dir = std::env::var("CABIN_REGISTRY_FIXTURES")
        .expect("CABIN_REGISTRY_FIXTURES must point at the gen-fixtures output directory");
    let mut checked = 0;
    ...
    assert!(
        checked >= 3,
        "expected at least the three gen-fixtures pairs in {dir}, found {checked}"
    );
}
