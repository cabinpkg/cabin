...
//!
//! `PUT /api/v1/sessions/tokens` trades a GitHub access token (from the
//! CLI's device flow) for a 12-hour `session` bearer token. The
//! host-testable parts live here: the GitHub `/user` lookup behind
...
/// The `GET /user` field the mint reads: the numeric id alone - stable
/// across renames, the identity the allowlist and `identities` key on.
#[derive(Deserialize)]
struct GithubUser {
    id: i64,
}
...
/// Production is the wasm glue's authenticated `GET /user` against
/// `GITHUB_API_BASE`; tests substitute a fake.
...
/// Parses a `GET /user` response body to the numeric id; unknown fields
/// are GitHub's business.
pub fn parse_github_user_id(body: &[u8]) -> Option<i64> {
    serde_json::from_slice::<GithubUser>(body)
        .ok()
        .map(|user| user.id)
}
...
pub async fn resolve_github_id(
    ...
    // Graphic-ASCII only: a JSON string can smuggle CR/LF into the
    // outbound Authorization header, and the header layer's exception
    // text is not proven to omit the value - refuse before it exists.
...
mod tests {
    ...
    impl GithubUserProvider for FakeGithub {
        async fn user_id(&self, github_token: &str) -> Result<i64, String> {
            match self {
                ...
                Self::Knows { .. } => Err("github /user refused the token".to_owned()),
                Self::Broken => Err("github /user fetch failed".to_owned()),
            }
        }
    }
    ...
    fn the_body_grammar_is_exact() {
        ...
        // could not be a GitHub token (empty, or carrying bytes a JSON
        // escape can smuggle into a header) all read as an absent
        // credential.
        for body in [
    ...
    fn the_provider_answer_passes_through_verbatim() {
        ...
        assert_eq!(
            ...
            Err("github /user refused the token".to_owned())
        );
        assert_eq!(
            block_on(resolve_github_id(
            ...
            Err("github /user fetch failed".to_owned())
        );
    }

    #[test]
    fn the_user_body_parses_on_the_id_alone() {
        // GitHub's real /user answer carries dozens of fields.
        assert_eq!(
            parse_github_user_id(br#"{"login": "octocat", "id": 583231, "type": "User"}"#),
            Some(583_231)
        );
        assert_eq!(parse_github_user_id(br#"{"login": "octocat"}"#), None);
        assert_eq!(parse_github_user_id(b"not json"), None);
    }
    ...
    fn a_github_failure_leaves_no_partial_state() {
        ...
        assert_eq!(refused, Err("github /user fetch failed".to_owned()));
        assert_eq!(count_tokens(&conn), 0);
    }
