...
# idempotent re-publish, the changed-bytes conflict and its new-revision
# opt-in, yank state
...
# revision_of <archive>: the packaging revision id, the leading 16 hex of
# the archive's SHA-256. The immutable unit is (scope, name, version,
# revision), and both the artifact route and the canonical source path
# spell the id out.
revision_of() { shasum -a 256 "$1" | cut -d' ' -f1 | cut -c1-16; }

# retarget_hash <old-hash> <new-hash>: rewrite a canonical metadata
# document on stdin onto an archive with <new-hash>. Two substitutions,
# because the document names the digest twice at two lengths: `checksum`
# carries all 64 hex, `source.path` only the leading 16. The full digest
# must go first - the prefix rewrite would otherwise corrupt it.
retarget_hash() { sed -e "s/$1/$2/g" -e "s/${1:0:16}/${2:0:16}/g"; }
...
# `cargo gen-fixtures`) doubles as the smoke publish payload verbatim:
# gen-fixtures authors its packages under the `smoke` scope precisely
# so the pair this flow publishes matches the scope claimed below.
scope="smoke"
name="withdep"
version="0.2.0"
...
rev="$(revision_of "$fixture_archive")"
artifact_path="/artifacts/$scope/$name/$scope-$name-$version-$rev.zip"
work="$(mktemp -d)"
...
# Everything below derives the artifact routes from $rev, so a fixture
# regenerated onto other bytes must fail here rather than turn the
# route assertions into 404s that still pass.
grep -qF "$scope-$name-$version-$rev.zip" "$fixture_metadata" \
  || fail "the frozen fixture's source path does not name revision $rev"
...
# shape: name, version, revision, checksum, published_at, metadata).
listing_entry() {
  node -e '
    const fs = require("fs");
    ...
    fs.writeFileSync(process.argv[4], JSON.stringify({
      name: v.name, version: v.version, revision: v.revision,
      checksum: v.checksum, published_at: v.published_at, metadata: v.metadata,
    }));
...
# The body must be a *semantically valid* rejected verdict - bound to the
# pending revision's checksum and publish event (every verdict requires
# both), padded past the cap with whitespace inside the JSON document -
# so an uncapped handler would parse and apply it, failing the
# pending-row checks below.
wcheck "/api/v1/admin/versions?status=pending" 200
listing_entry "$body" "$scope/$name" "$version" "$work/entry-precap.json"
pending_published_at="$(node -pe \
  'JSON.parse(require("fs").readFileSync(process.argv[1], "utf8")).published_at' \
  "$work/entry-precap.json")"
printf '{"verdict":"rejected","reason":"oversized","checksum":"%s","published_at":"%s"%4097s}' \
  "$blob_hash" "$pending_published_at" '' \
...
expect_body "\"name\":\"$scope/$name\""
expect_body '"0.2.0"'
# The composed entry names the revision it serves, and lists it in the
# per-version revisions map with the canonical source path.
expect_body "\"revision\":\"$rev\""
expect_body "../../artifacts/$scope/$name/$scope-$name-$version-$rev.zip"
...
# every verdict now requires: the conflict must be the transition, not
# a missing binding.
node -e '
  const fs = require("fs");
  const entry = JSON.parse(fs.readFileSync(process.argv[1], "utf8"));
  ...
  }));' "$work/entry.json" "$work/verdict-rejected.json"
...
new_rev="$(revision_of "$work/tampered.zip")"
tampered_artifact_path="/artifacts/$scope/$name/$scope-$name-$version-$new_rev.zip"
retarget_hash "$old_hash" "$new_hash" <"$fixture_metadata" >"$work/tampered.json"
frame "$work/tampered.json" "$work/tampered.zip" "$work/tampered.bin"
wrequest PUT "$publish_path" "$work/tampered.bin" 409
expect_body 'new-revision'

step "the opt-in publishes the changed bytes as a pending respin"
# The same refused pair, opted in: a new packaging revision of the same
# version, pending like any other first publish - and invisible to reads
# until it is verified, so what the registry serves cannot move.
wrangler d1 execute DB --local --command "
  UPDATE tokens SET rl_tokens = NULL, rl_updated_at = NULL WHERE id = 'smoke';" >/dev/null
wrequest PUT "$publish_path?new-revision=true" "$work/tampered.bin" 201
expect_body "\"revision\":\"$new_rev\""
expect_body '"verification":"pending"'
check "$package_path" 200
expect_body "\"revision\":\"$rev\""
! grep -qF "$new_rev" "$body" \
  || fail "a pending respin leaked into the package document: $(cat "$body")"
check "$tampered_artifact_path" 404
as_verifier
check "$tampered_artifact_path" 200
wcheck "/api/v1/admin/versions?status=pending" 200
expect_body "\"revision\":\"$new_rev\""
# Rejected right here: the respin has served its purpose, and leaving it
# pending would put a second candidate in front of every later listing
# and stuck-pending alert.
listing_entry "$body" "$scope/$name" "$version" "$work/entry-respin.json"
node -e '
  const fs = require("fs");
  const entry = JSON.parse(fs.readFileSync(process.argv[1], "utf8"));
  fs.writeFileSync(process.argv[2], JSON.stringify({
    verdict: "rejected", reason: "smoke respin",
    checksum: entry.checksum, published_at: entry.published_at,
  }));' "$work/entry-respin.json" "$work/verdict-respin.json"
wrequest PATCH "/api/v1/admin/versions/$scope/$name/$version" "$work/verdict-respin.json" 200
expect_body "\"revision\":\"$new_rev\""
...
# Identical bytes, so this version's first revision carries the same id.
artifact2_path="/artifacts/$scope/$name/$scope-$name-$version2-$rev.zip"
...
# Different bytes are a different revision, but the version's only other
# revision is the rejected one - nothing live to supersede - so this
# needs no new-revision opt-in.
tamper_zip "$fixture_archive" "$work/replacement.zip" 2
replacement_hash="$(shasum -a 256 "$work/replacement.zip" | cut -d' ' -f1)"
replacement_rev="$(revision_of "$work/replacement.zip")"
artifact2_replacement_path="/artifacts/$scope/$name/$scope-$name-$version2-$replacement_rev.zip"
retarget_hash "$blob_hash" "$replacement_hash" \
  <"$work/withdep-0.2.1.json" >"$work/replacement.json"
frame "$work/replacement.json" "$work/replacement.zip" "$work/replacement.bin"
wrequest PUT "$publish2_path" "$work/replacement.bin" 201
expect_body "\"revision\":\"$replacement_rev\""
expect_body '"verification":"pending"'
...
listing_entry "$body" "$scope/$name" "$version2" "$work/entry-replacement.json"
node -e '
  const fs = require("fs");
  const entry = JSON.parse(fs.readFileSync(process.argv[1], "utf8"));
  fs.writeFileSync(process.argv[2], JSON.stringify({
    verdict: "rejected", reason: "smoke rejection",
    checksum: entry.checksum, published_at: entry.published_at,
  }));' "$work/entry-replacement.json" "$work/verdict-rejected-replacement.json"
wrequest PATCH "$verdict2_path" "$work/verdict-rejected-replacement.json" 200
as_publisher
[[ "$(stored_bytes)" == "$before_bytes" ]] \
...
# Byte-identical, so this revives the rejected revision in place rather
...
as_verifier
check "$artifact2_replacement_path" 200
as_publisher
...
sed "s/0\\.2\\.0/$profile_version/g" "$fixture_metadata" |
  retarget_hash "$blob_hash" "$notzip_hash" >"$work/notzip.json"
...
profile_artifact_path="/artifacts/$scope/$name/$scope-$name-$profile_version-$(revision_of "$work/evil.zip").zip"
sed "s/0\\.2\\.0/$profile_version/g" "$fixture_metadata" |
  retarget_hash "$blob_hash" "$evil_hash" >"$work/evil.json"
...
publish_min_version() { # <version> <zip>: sed the fixture metadata onto it
  ...
  sed "s/0\.2\.0/$v/g" "$fixture_metadata" |
    retarget_hash "$blob_hash" "$hash" >"$work/iso-$v.json"
...
# load_artifact <version>: the artifact route of a load-leg fixture,
# revision and all.
load_artifact() {
  printf '/artifacts/%s/%s/%s-%s-%s-%s.zip' \
    "$scope" "$name" "$scope" "$name" "$1" "$(revision_of "$work/load-$1.zip")"
}
...
iso_verified_artifact="/artifacts/$scope/$name/$scope-$name-$iso_verified_version-$(revision_of "$work/iso-verified.zip").zip"
iso_pending_artifact="/artifacts/$scope/$name/$scope-$name-$iso_pending_version-$(revision_of "$work/iso-pending.zip").zip"
...
sed "s/0\.2\.0/0.6.0/g" "$fixture_metadata" |
  retarget_hash "$blob_hash" "$blocked_hash" >"$work/iso-blocked.json"
