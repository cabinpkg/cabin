...
fn wipe_guard(smoke: &mut Smoke, registry_dir: &Path) -> Result<()> {
    ...
    if xtask_registry_admin::results(&generation)?.len() != 1 {
...
fn set_launched(value: &str) -> Result<()> {
    ...
    xtask_registry_admin::output(&mut xtask_registry_admin::wrangler(&[
        "d1",
        "execute",
        "DB",
        "--local",
...
/// which is what the comparisons and every interpolated diagnostic in
/// this leg read.
