...
//! late-prepared foundation ports).  Resolve-level tests need no
//! toolchain; the build-level tests guard on build tools.
...
/// A foundation port pulled in by a transitively-activated fork is
/// prepared after resolution, so its `links` claim is invisible to
/// `cabin resolve`; the building commands' final-graph check is
/// what refuses its collision with a registry claimant.
#[test]
fn late_port_claim_collides_at_build() {
    require_c_and_cxx_build_tools();
    let dir = TempDir::new().unwrap();
    let repo = FakePortRepo::new(dir.path());
    let zport = repo
        .port("zport", "1.0.0")
        .archive_prefix("zport-1.0.0")
        .file("zport.h", "#pragma once\nint zp(void);\n")
        .file(
            "zport.c",
            "#include \"zport.h\"\nint zp(void) { return 1; }\n",
        )
        .overlay_manifest(
            r#"[package]
name = "zport"
...
"#,
        ...
        .build();
    let _server = FakeArchiveServer::new().serve(&zport.archive).start();
...
[dependencies]
zport = { port-path = "../ports/zport/1.0.0" }
...
"#,
    ...
    let output = build_assert(dir.path(), &manifest).failure();
    let stderr = String::from_utf8_lossy(&output.get_output().stderr);
    for needle in [COLLISION_PHRASE, "\"z\"", "zport", "acme/zzz"] {
        assert!(stderr.contains(needle), "expected {needle:?} in: {stderr}");
    }
}

/// A late-prepared port may not declare a versioned registry
/// dependency: resolution has already run, so instead of silently
/// dropping the edge (or trusting a same-named fetch without
/// checking the requirement) the build refuses with a typed
/// diagnostic naming the port and the dependency.
#[test]
fn late_port_registry_dep_refused() {
    require_c_and_cxx_build_tools();
    let dir = TempDir::new().unwrap();
    let repo = FakePortRepo::new(dir.path());
    let regport = repo
        .port("regport", "1.0.0")
        .archive_prefix("regport-1.0.0")
        .file("regport.c", "int rp(void) { return 1; }\n")
        .overlay_manifest(
            r#"[package]
name = "regport"
version = "1.0.0"
...
"#,
        ...
        .build();
    let _server = FakeArchiveServer::new().serve(&regport.archive).start();
...
[dependencies]
regport = { port-path = "../ports/regport/1.0.0" }
...
"#,
    ...
    let stderr = String::from_utf8_lossy(&output.get_output().stderr);
    for needle in [
        "ports discovered after resolution cannot resolve registry dependencies",
        "regport",
        "acme/ccc",
    ] {
        assert!(stderr.contains(needle), "expected {needle:?} in: {stderr}");
