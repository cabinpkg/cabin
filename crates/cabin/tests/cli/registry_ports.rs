...
//! pipeline (`xtask-port-publish`) from the *committed* manifests,
//! so these tests consume the same canonical packages the
//! hosted `cabin-ports` scope serves: scoped lowercase names,
//! native-artifact target keys (`zlib`'s sole library target
//! publishes as `z`), scoped inter-port dependencies, and stamped
...
/// Build the fixture port set from the committed manifests (fake
/// upstream sources under each port's real layout), run it through
/// the publisher pipeline, and publish it into a file registry:
///
...
/// dependency edge: SQLite compiles with `SQLITE_THREADSAFE=0` and
...
fn provenance_round_trips_committed_pin_into_index_and_archived_manifest() {
    ...
    // The libpng entry's dependency map carries the scoped edge, so
    // transitive resolution needs no port knowledge.
    assert_eq!(
