...
/// Whatever would let an edit to an alias or its crate stop reaching the
/// job it feeds.
///
/// A filtered workflow that runs `cargo <alias>` only runs when its
/// filter matches, so the alias file and the tool's crate have to be in
/// that filter - otherwise changing either one skips the very job that
...
fn alias_consumer_problems(repo_root: &Path) -> Result<Vec<String>> {
    ...
    for workflow in workflows {
        let name = workflow.file_name().unwrap_or_default().to_string_lossy();
        let text = std::fs::read_to_string(&workflow)
            .with_context(|| format!("read {}", workflow.display()))?
            .replace("\r\n", "\n");
        let filters = path_filters(&text);
        if filters.is_empty() {
            continue;
        }
        for (alias, value) in aliases.iter().filter(|(alias, _)| runs_alias(&text, alias)) {
            // An alias off the convention is already reported; there is
            // no crate to require a filter for.
...
/// Every `paths:` / `paths-ignore:` list in a workflow, in both the block
/// and flow spellings, with the keyword each belongs to.
fn path_filters(text: &str) -> Vec<(&str, Vec<String>)> {
    let strip = |item: &str| item.trim().trim_matches(['"', '\'']).to_owned();
    ...
    while let Some(line) = lines.next() {
        ...
        if keyword != "paths" && keyword != "paths-ignore" {
            continue;
        }
        let mut items: Vec<String> = rest
            ...
            .collect();
        // A block sequence may sit at the key's own indent or deeper,
        // and these lists are commented heavily between their items.
        while let Some(next) = lines.peek() {
            ...
            if !inner.starts_with("- ") || next.len() - inner.len() < indent {
                break;
            }
            items.push(strip(&inner[2..]));
            lines.next();
        }
        filters.push((keyword, items));
    }
    filters
}
