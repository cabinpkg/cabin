...
fn build(args: &BuildArgs, reporter: Reporter) -> Result<()> {
    ...
    let registry: Vec<RegistryPackageSource> = if has_versioned {
        ...
        crate::config_glue::enforce_offline_post_replacement(build_offline, &resolved_locator)?;
...
fn fetch(args: &FetchArgs, reporter: Reporter) -> Result<()> {
    ...
    crate::config_glue::enforce_offline_post_replacement(fetch_offline, &resolved_locator)?;
    let (replaced_path, replaced_url) =
        crate::patch_glue::locator_to_index_inputs(&resolved_locator.resolved);
    ...
    emit_fetch_output(&pipeline.fetched, args.format, &cache_dir, &manifest_path)?;
    Ok(())
}
...
/// Resolve the cache directory using --cache-dir, $CABIN_CACHE_DIR, or
/// the package-local default `<root_dir>/.cabin/cache`.
pub(crate) fn cache_dir_for(manifest_path: &Path, override_dir: Option<&Path>) -> Result<PathBuf> {
    ...
    if let Some(env) = std::env::var_os("CABIN_CACHE_DIR") {
    ...
    Ok(root_dir.join(".cabin").join("cache"))
}
